...
```text
浏览器访问应用的精确 Custom Domain
        ↓
边缘网关按 Host 解析应用 alias
        ↓
无有效会话 → 密码验证 → 签发 alias 绑定 Cookie
        ↓
根据服务端 ROUTE_PROJECTS_JSON 构造已配置的上游 URL
        ↓
安全代理静态响应，浏览器继续停留在统一入口域名
```
...
- 请求查询参数和安全路径会传递给上游。
- 只转发有限的请求头，不把网关会话 Cookie 或客户端 Authorization 发送给上游。
...
- 需要把 POST、PUT、DELETE 等同源 API 请求继续代理到上游的应用；
